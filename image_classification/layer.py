...
class fully_connected:
    ...
    def init_param(self):
        self.kernel = np.random.uniform(
            low = -np.sqrt(6.0/(self.in_channels)),
            high = np.sqrt(6.0/(self.in_channels)),
...
class conv2d:
    ...
    def init_param(self):
        self.kernel = np.random.uniform(
            low = -np.sqrt(6.0/(self.in_channels * self.kernel_h * self.kernel_w)),
            high = np.sqrt(6.0/(self.in_channels * self.kernel_h * self.kernel_w)),
    ...
    def forward(self, in_tensor):
        ...
        if self.same:
            in_tensor = conv2d.pad(in_tensor, int((self.kernel_h-1)/2), int((self.kernel_w-1)/2))
        
        self.in_tensor = in_tensor.copy()
        
        self.out_tensor = conv2d.convolution(in_tensor, self.kernel, self.stride)
    ...
    def backward(self, out_diff_tensor, lr):
        ...
        kernel_diff = conv2d.convolution(self.in_tensor.transpose(1,0,2,3), extend_out.transpose(1,0,2,3))
        kernel_diff = kernel_diff.transpose(1,0,2,3)
        
        padded = conv2d.pad(extend_out, self.kernel_h-1, self.kernel_w-1)
        ...
        self.in_diff_tensor = conv2d.convolution(padded, kernel_trans.transpose(1,0,2,3))
